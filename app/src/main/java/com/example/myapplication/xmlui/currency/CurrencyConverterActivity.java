...
public class CurrencyConverterActivity extends AppCompatActivity {
    ...
    private final ActivityResultLauncher<Intent> fromCurrencyPickerLauncher =
        registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() != RESULT_OK || result.getData() == null) {
                return;
            }
            String code = safe(result.getData().getStringExtra(CurrencyPickerActivity.EXTRA_SELECTED_CURRENCY));
            if (code.isEmpty()) {
                return;
            }
            selectedFrom = code;
    ...
    private final ActivityResultLauncher<Intent> toCurrencyPickerLauncher =
        registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() != RESULT_OK || result.getData() == null) {
                return;
            }
    ...
    private void applySnapshot(ExchangeRateSnapshot snapshot) {
        ...
        if (snapshot != null && snapshot.getRates() != null && !snapshot.getRates().isEmpty()) {
            ...
            supportedCurrencies.addAll(SupportedCurrencyCatalog.filterCodesWithLogos(this, merged));
        }
        if (supportedCurrencies.isEmpty()) {
            seedLocalCurrencies();
        }
        ensureSelectedCurrencies();
        btnConvert.setEnabled(snapshot != null && !supportedCurrencies.isEmpty());
        updateCurrencySelectionUi();
        updateRatePreview();
    }
    ...
    private void seedLocalCurrencies() {
        supportedCurrencies.clear();
        supportedCurrencies.addAll(SupportedCurrencyCatalog.defaultCodes(this));
    }

    private void ensureSelectedCurrencies() {
        if (supportedCurrencies.isEmpty()) {
            selectedFrom = "USD";
            selectedTo = "VND";
            return;
    ...
    private void showCurrencyPickerDialog(boolean pickingFromCurrency) {
        ...
        intent.putExtra(CurrencyPickerActivity.EXTRA_SELECTED_CURRENCY, selectedCode);
        intent.putExtra(CurrencyPickerActivity.EXTRA_PICKER_TITLE, getString(R.string.picker_title_currency));
        if (pickingFromCurrency) {
            fromCurrencyPickerLauncher.launch(intent);
        } else {
            toCurrencyPickerLauncher.launch(intent);
        }
    }
    ...
    private void bindCurrencySelection(ImageView flagView, TextView codeView, String currencyCode) {
        String code = CurrencyRateUtils.normalizeCurrency(currencyCode);
        codeView.setText(code);
        CurrencyLogoUtils.bindLogo(flagView, code, R.drawable.ic_wallet_currency);
    }
