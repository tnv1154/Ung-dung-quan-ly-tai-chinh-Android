...
public final class AppNotificationCenter {
    ...
    static void notifyBudgetExceeded(Context context, String category, double spent, double limit) {
        ...
        NotificationManagerCompat.from(context).notify(budgetNotificationId(displayCategory), builder.build());
    }
    ...
    private static int budgetNotificationId(String category) {
        return NOTIFICATION_ID_BUDGET_BASE + Math.abs(category.hashCode() % 700);
    }
