...
public class AuthActivity extends AppCompatActivity {
    ...
    private View layoutPasswordRules;
    private View ruleMixedCaseIndicator;
    private View ruleDigitIndicator;
    private View ruleLengthIndicator;
    private TextView tvRuleMixedCase;
    private TextView tvRuleDigit;
    private TextView tvRuleLength;
    ...
    private void bindViews() {
        ...
        layoutPasswordRules = findViewById(R.id.layoutAuthPasswordRules);
        ruleMixedCaseIndicator = findViewById(R.id.viewRuleMixedCaseIndicator);
        ruleDigitIndicator = findViewById(R.id.viewRuleDigitIndicator);
        ruleLengthIndicator = findViewById(R.id.viewRuleLengthIndicator);
        tvRuleMixedCase = findViewById(R.id.tvAuthRuleMixedCase);
        tvRuleDigit = findViewById(R.id.tvAuthRuleDigit);
        tvRuleLength = findViewById(R.id.tvAuthRuleLength);
    ...
    private void renderMode() {
        ...
        layoutPasswordRules.setVisibility(registerMode ? View.VISIBLE : View.GONE);
    ...
    private void handleEmailAuth() {
        String email = etAccount.getText() != null ? etAccount.getText().toString().trim() : "";
        ...
        if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            tvAuthError.setText(R.string.auth_error_invalid_email);
            tvAuthError.setVisibility(View.VISIBLE);
            return;
        }
        ...
        if (registerMode) {
            if (!AuthPasswordPolicy.isValid(password)) {
                tvAuthError.setText(R.string.auth_error_password_policy);
                tvAuthError.setVisibility(View.VISIBLE);
                return;
            }
    ...
    private void updatePasswordRuleIndicators() {
        AuthPasswordPolicy.RuleState ruleState = AuthPasswordPolicy.evaluate(
            etPassword.getText() == null ? "" : etPassword.getText().toString()
        );
        applyPasswordRuleState(ruleMixedCaseIndicator, tvRuleMixedCase, ruleState.hasMixedCase());
        applyPasswordRuleState(ruleDigitIndicator, tvRuleDigit, ruleState.hasDigit());
        applyPasswordRuleState(ruleLengthIndicator, tvRuleLength, ruleState.hasMinLength());
    }

    private void applyPasswordRuleState(View indicator, TextView label, boolean passed) {
        indicator.setBackgroundColor(ContextCompat.getColor(this, passed ? R.color.success_green : R.color.divider));
        label.setTextColor(ContextCompat.getColor(this, passed ? R.color.success_green : R.color.text_secondary));
    }
