...
public class AddTransactionActivity extends AppCompatActivity {
    ...
    private void renderFinanceState(@NonNull FinanceUiState state) {
        ...
        if (pendingSubmit && errorMessage != null) {
            showError(errorMessage);
            pendingSubmit = false;
            financeViewModel.clearError();
        } else if (errorMessage != null && isPermissionDenied(errorMessage)) {
            financeViewModel.clearError();
        }
    }
    ...
    private void styleModeSelector(boolean filled) {
        if (filled) {
            btnModeSelector.setBackgroundResource(R.drawable.bg_add_mode_filled);
        } else {
            btnModeSelector.setBackgroundResource(R.drawable.bg_add_mode_outline);
        }
    ...
    private void chooseDestinationWallet() {
        ...
        for (int i = 0; i < options.size(); i++) {
            names[i] = options.get(i).getName() + " • " + UiFormatters.money(options.get(i).getBalance());
        }
        new MaterialAlertDialogBuilder(this)
    ...
    private void submitTransaction() {
        ...
        if (selectedMode == Mode.ADJUST) {
            ...
            pendingSubmit = true;
            financeViewModel.adjustWalletBalance(selectedSourceWallet.getId(), actual, note, selectedTimestamp);
